<?php
/**
 * Create a secret key for OneAccess site communication.
 *
 * @package OneAccess
 */
...
class Secret_Key {
	...
	public static function generate_secret_key( bool $is_regenerate = false ): string {
		$secret_key = get_option( Constants::ONEACCESS_API_KEY );
		if ( empty( $secret_key ) || $is_regenerate ) {
			$secret_key = self::generate_key();
			// Store the secret key in the database.
			$is_key_updated = update_option( Constants::ONEACCESS_API_KEY, $secret_key, false );
